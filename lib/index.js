...
// 静默检查 GitHub 最新版本,结果写入 UPDATE_CHECK 供面板读取
...
function apply(ctx) {
    ...
    async function checkForUpdate() {
      ...
      try {
        ...
        if (hasUpdate) reg.log('info', 'update-check', `发现新版本 v${latestTag}(当前 ${VERSION})`)
      } catch (e) {
    ...
    async function ensureZhipuProviderDeclaration() {
      try {
        ...
        if (usable) {
          ...
          if (!zhipuCred.configured) {
            log('info', 'provider-ensure', '已有可用的视觉模型且未配置 ZHIPU_GLM_API_KEY,跳过 zhipu-glm 自动补写(配置 GLM key 后会自动补上)')
            return false
          }
          log('info', 'provider-ensure', '检测到 ZHIPU_GLM_API_KEY 已配置,补写 zhipu-glm provider 声明以启用 GLM 视觉模型')
        } else {
          log('info', 'provider-ensure', '没有可用的视觉模型(新机或全部停用),补写免费默认 zhipu-glm(glm-4.6v-flash)')
        }
        ...
        log('info', 'provider-ensure', '已自动补写 zhipu-glm provider 声明(api/baseURL/input 等,llm-pi-ai 适配器热更新路由)')
        return true
      } catch (e) {
    ...
    async function ensureAndRescan() {
      ...
      if (wrote) {
        log('info', 'provider-ensure', 'zhipu-glm 声明已补写,重新扫描模型列表…')
        await scan()
      }
      return wrote
    }
        ...
        for (const vm of scanResult.visionModels || []) {
          ...
          if (prev === undefined) {
            if (configured) {
              ...
              log('info', 'route-auto-enable', `${vm.provider}/${vm.id} 凭据已配置,自动启用`)
            }
          }
          // 之前已配且现在没了 → 关闭(保留给用户手动决定是否重配)
            ...
            if (!configured) {
              ...
              log('info', 'route-auto-disable', `${vm.provider}/${vm.id} 凭据消失,自动停用`)
            }
          }
        }
      }
    ...
    function recommendedOf() {
      if (!scanResult.ready) {
        // 未就绪时返回与就绪态一致的形状({provider, id}),避免下游 .model 显示 undefined
        return RECOMMENDED_VISION_ROUTES
          .filter((r) => isRouteEnabled(r.provider, r.model))
          .map((r) => ({ provider: r.provider, id: r.model }))
      }
    ...
    function registerHttpApi() {
      ...
      ctx.effect(() => ws.register({
        ...
        handler: async (req, res) => {
          ...
          const profileDir = require('node:path').join(require('node:os').homedir(), '.dsh', 'profiles', 'web')
          const result = spawnSync('dsh', ['plugin', '--profile', 'web', 'update', 'taishan-vision'], {
            cwd: profileDir, shell: true, stdio: 'pipe', timeout: 120000,
          })
          const stdout = (result.stdout || '').toString()
          const stderr = (result.stderr || '').toString()
          const exitCode = result.status ?? 1
          const ok = exitCode === 0
          reg.log(ok ? 'info' : 'error', 'upgrade', { exitCode, ok })
          json(res, ok ? 200 : 500, {
            ok,
            exitCode,
    ...
    /* ==================== [12] 启动 ==================== */
    // 启动链:扫描 → 视情况自动补写 zhipu-glm 声明并重扫。
    // 触发补写(v2.2):无可用视觉模型(新机/全部停用),或用户已配置 ZHIPU_GLM_API_KEY
    // ——否则(如本机自带 deepseek 视觉模型时)填了 GLM key 也看不到任何 GLM 模型。
    void (async () => {
      await scan()
      await ensureAndRescan()
      // 静默检查更新(5分钟缓存 + 每小时限3次,失败不报错)
