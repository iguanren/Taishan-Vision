...
// 检查 GitHub 最新版本,结果写入 UPDATE_CHECK 供面板读取(仅用户手动触发)
const UPDATE_CHECK = { checkedAt: null, hasUpdate: false, latest: null, error: null }
const VERSION = 'v5.0'
// 更新检查限流变量(模块级,checkForUpdate 仅在用户手动点击时调用)
let updateFirstCallTime = null
let updateCallCount = 0
const UPDATE_REQ_PER_HOUR = 3

function apply(ctx) {
    /* ==================== [0] 配置 ==================== */
    async function checkForUpdate() {
      const now = Date.now()
      if (UPDATE_CHECK.checkedAt && (now - UPDATE_CHECK.checkedAt) < 5 * 60 * 1000) return
      if (updateFirstCallTime && (now - updateFirstCallTime) >= 60 * 60 * 1000) {
        updateFirstCallTime = now; updateCallCount = 0
      }
      if (updateCallCount >= UPDATE_REQ_PER_HOUR) return
      updateCallCount++
      if (!updateFirstCallTime) updateFirstCallTime = now
      try {
        const resp = await fetch('https://api.github.com/repos/iguanren/taishan-vision/releases/latest', {
          headers: { 'User-Agent': `taishan-vision/${VERSION}` },
          signal: AbortSignal.timeout(5000),
        })
        if (!resp.ok) throw new Error(`HTTP ${resp.status}`)
        const data = await resp.json()
        const latestTag = (data.tag_name || '').replace(/^v/, '')
        const semver = (s) => s.split('.').map((n) => parseInt(n, 10) || 0)
        const cur = semver(VERSION.replace(/^v/, ''))
        const lte = semver(latestTag)
        const hasUpdate = lte[0] > cur[0] || (lte[0] === cur[0] && (lte[1] > cur[1] || (lte[1] === cur[1] && lte[2] > cur[2])))
        UPDATE_CHECK.hasUpdate = hasUpdate
        UPDATE_CHECK.latest = hasUpdate ? latestTag : null
        UPDATE_CHECK.checkedAt = Date.now()
        // 静默，不输出
      } catch (e) {
        UPDATE_CHECK.error = String(e.message).slice(0, 80)
        UPDATE_CHECK.checkedAt = Date.now()
        reg.log('warn', 'update-check', '检查更新失败:' + UPDATE_CHECK.error)
      }
    }

    function getUpdateStatus() {
      return { ...UPDATE_CHECK, current: VERSION.replace(/^v/, '') }
    }
    ...
    // 启动补写:扫描成功后,当"没有可用视觉模型"或"用户已配置 ZHIPU_GLM_API_KEY"时自动写入 zhipu-glm 声明(已存在则跳过,不覆盖用户自定义)
    async function ensureZhipuProviderDeclaration() {
      try {
          // 扫描未完成时静默跳过,不输出日志
          if (!scanResult.ready) return false
        // v2.2:补写判断从"完全没有视觉模型"放宽为"没有可用视觉模型"或"用户已配置 GLM key"。
        // 复现场景:本机带 deepseek 视觉模型时旧逻辑永远跳过补写,用户在面板填了
        // ZHIPU_GLM_API_KEY 也看不到任何 GLM 模型(provider 都没声明,key 无处生效)。
        const usable = hasVisionModel()
        if (usable) {
          const zhipuCred = await describeCredential('ZHIPU_GLM_API_KEY')
          if (!zhipuCred.configured) return false
          // 静默,不输出
        } else {
          // 静默,不输出
        }
        ...
        // rc.1:llm-pi-ai 以 settings 校验器拒绝不可服务的 section(settings-rejected),
        // 失败时保持现状并记录,引导文案会提示手动添加,避免静默失败。
        log('warn', 'provider-ensure', '自动补写失败(可能被 llm-pi-ai 适配器拒绝或 settings 服务异常):' + String(e))
        return false
      }
    }
    ...
    async function ensureAndRescan() {
      const wrote = await ensureZhipuProviderDeclaration()
      if (wrote) {
        await scan()
      }
      return wrote
    ...
    function ensureRouteKeys() {
      for (const key of knownRouteKeys()) {
        ...
        // v2.3:凭据感知默认——只有 configured===true(明确配了 key)才默认启用,
    ...
    async function scan() {
      ...
      scanPromise = (async () => {
        ...
        try {
          ...
          scanResult = result
          ensureRouteKeys()
          // v2.3:扫描完成后检测凭据状态变化——若某个 provider 从「未知」变为「已配置」,
          // 自动开启对应路由(用户只需在面板存一次 key,无需手动点开关)
    ...
    // v2.3:凭据状态变化时自动启停路由。
    // missingCredentials 是本轮扫描结果;与内存中当前 routesEnabled 对比,
    // 若某 provider 的 configured 从 null/undefined 变成 true,自动打开对应路由;
    // 变成 false 则关闭。只在首次感知到变化时触发,避免反复 toggle。
    function reconcileRoutesOnKeyChange(missingCreds) {
      ...
      for (const mc of missingCreds) {
        ...
        // 遍历所有 vision model 中找到属于该 provider 的 route
    ...
    if (llm && typeof llm.resolveModelInfo === 'function' && !llm[wrapKey]) {
      ...
      llm.resolveModelInfo = async function (provider, model, signal) {
        ...
        try {
          if (info && typeof info === 'object') {
            ...
            // 修复:无可用视觉路由(如未配置 ZHIPU_GLM_API_KEY)时,
            // 不给纯文本模型伪造 image 能力,避免模型选择器错误显示「+ 自动识图」标签。
      ...
      llm[wrapKey] = state
      // 启动日志精简:正常情况静默,仅在 warn/error 级别输出
    ...
    function guidanceText(reason) {
      ...
      if (reason === 'all-disabled') {
        lines.push('2. 当前所有识图模型均已被关闭:请在「设置 → 插件 → 插件配置 → 泰山识图」卡片中勾选至少一个识图模型;')
        lines.push('   推荐启用 zhipu-glm/glm-4.6v-flash(免费;粘贴智谱 API Key 后即自动启用)。')
      } else if (reason === 'missing-provider') {
        lines.push('2. 插件已自动加入提供方 zhipu-glm(glm-4.6v-flash / glm-4.1v-thinking-flash,免费);')
        lines.push('   只需在「设置 → 插件 → 插件配置 → 泰山识图」粘贴智谱 API Key(ZHIPU_GLM_API_KEY)即完成配置;')
        lines.push('   若自动加入失败,可在「设置 → 模型」中手动添加提供方 zhipu-glm(基址 https://open.bigmodel.cn/api/paas/v4)。')
      } else if (reason === 'missing-credential') {
        lines.push('2. 提供方已就绪,只差 API Key:在「设置 → 插件 → 插件配置 → 泰山识图」粘贴智谱 API Key 即可,')
        lines.push('   或写入 ~/.dsh/.credentials.yaml 的 ZHIPU_GLM_API_KEY(或设置同名环境变量);')
      } else {
        lines.push('2. 在 DeepSeek Harness「设置 → 模型」中添加支持图片输入的模型,')
        lines.push('   智能识别任意视觉模型;推荐在「设置 → 插件 → 插件配置 → 泰山识图」配置智谱 GLM 免费模型(ZHIPU_GLM_API_KEY);')
      }
      lines.push('3. 配置多个视觉模型时可在「设置 → 插件 → 插件配置 → 泰山识图」中调整路由顺序与启停;')
    ...
    function resolveImageRef(agent, attachmentId) {
      ...
      if (session) {
        // rc.1+:dsh-session 改为快照 API(snapshotEvents()),不再暴露 .events 数组
        try {
    ...
    const tool = defineTool({
      ...
      async execute(args, exec) {
        const startedAt = Date.now()
        if (!config.enabled) {
          throw new Error('泰山识图已停用:请在「设置 → 插件 → 插件配置 → 泰山识图」卡片中重新启用。')
        }
    ...
    function registerHttpApi() {
      ...
      ctx.effect(() => ws.register({
        kind: 'exact',
        path: '/api/taishan/check-update',
        handler: async (req, res) => {
          if (req.method !== 'GET') { json(res, 405, { ok: false, error: 'method not allowed: use GET' }); return }
          await checkForUpdate()
          json(res, 200, { ok: true, ...getUpdateStatus() })
        },
      }), 'taishan: check-update endpoint')
      ctx.effect(() => ws.register({
        kind: 'exact',
        path: '/api/taishan/upgrade',
        handler: async (req, res) => {
          if (!requirePost(req, res)) return
          // 直接计算 profile 目录,不依赖 dsh-app-boot(避免模块解析问题)
          const profileDir = join(homedir(), '.dsh', 'profiles', 'web')
          const result = spawnSync('dsh', ['plugin', '--profile', 'web', 'update', 'taishan-vision'], {
            cwd: profileDir, shell: false, stdio: 'pipe', timeout: 120000,
          })
          const stdout = (result.stdout || '').toString()
          const stderr = (result.stderr || '').toString()
          const exitCode = result.status ?? 1
          const ok = exitCode === 0
          if (!ok) reg.log('error', 'upgrade', { exitCode, ok })
          json(res, ok ? 200 : 500, {
            ok,
            exitCode,
            output: (stdout + stderr).trim().slice(-2000),
            version: ok ? VERSION.replace(/^v/, '') : null,
          })
        },
      }), 'taishan: upgrade endpoint')
      ctx.effect(() => ws.register({
        kind: 'exact',
        ...
        handler: async (req, res) => {
          ...
          req.on('end', async () => {
            ...
            try {
              ...
              // zhipu-glm(如本机已有 deepseek 视觉模型时不会自动声明),立即补写+重扫,
              // 填 key 后马上能看到 GLM 模型,无需重启。
              await scan()
              await ensureAndRescan()
    ...
    /* ==================== [11.5] 插件配置卡片命名空间(settings namespace) ==================== */
    // 泰山识图面板在浏览器端以「插件配置」卡片形式渲染(与 modsearch 同机制):
    // 客户端把卡片注册到 settings.plugin.item 插槽(key="taishan-vision"),
    // 「插件配置」页按 settings 命名空间分发卡片。Host 端必须先注册
    // "taishan-vision" 命名空间,卡片才会被渲染(否则静默不显示)。
    // 本命名空间是空的 passthrough——面板配置实际走内存 config + HTTP 端点,
    // 命名空间只起"让卡片被分发"的作用,与 modsearch 完全一致。
    // 用 ctx.inject(['settings']) 的 scoped 注入注册(与 modsearch 第 90-101 行相同):
    // 闭包在 settings 服务真正可用时执行;settings 缺失的环境(如 headless)自动跳过。
    if (typeof ctx.inject === 'function') {
      ctx.inject(['settings'], (scope) => {
        try {
          ...
          scope.settings.register('taishan-vision', passThrough, { base: {} })
          // 通知浏览器端 settings describe 镜像重读:ui-settings 客户端的
          // SettingsDescribeMirror 订阅 settings/document-updated 远端事件后
          // 会 mirror.load() 重拉 host describe(),新命名空间随快照进入
          // 「插件配置」Tab 的 served 集合,卡片随即被渲染。
          // 不广播的话镜像停在注册前快照,卡片永远不出现。
          try {
            ctx.events && ctx.events.emit('settings/document-updated', 'taishan-vision', 0)
          } catch (_) { /* 广播失败不影响主流程 */ }
        } catch (e) {
          console.error('[泰山识图] settings 命名空间注册失败:' + String(e))
        }
      })
    } else if (settingsSvc && typeof settingsSvc.register === 'function') {
      // 兜底:ctx.inject 不可用时退回顶层直用(settings 已在 inject 声明里,理论上不会走到这)
      try {
        const passThrough = (value) => ({ ...(value ?? {}) })
        passThrough.toJSON = () => ({
          uid: 0,
        ...
        settingsSvc.register('taishan-vision', passThrough, { base: {} })
        try { ctx.events && ctx.events.emit('settings/document-updated', 'taishan-vision', 0) } catch (_) { /* 兜底路径广播失败不影响主流程 */ }
      } catch (e) {
        console.error('[泰山识图] settings 命名空间注册失败:' + String(e))
      }
    }
    ...
    /* ==================== [12] 启动 ==================== */
    // v2.5:扫描与HTTP API注册并行执行,不再串行等待扫描完成才注册接口,
    // 缩短面板可访问前的等待时间。
    void (async () => {
