...
function apply(ctx) {
    ...
    /* ==================== [12] 启动 ==================== */
    // 启动链:扫描 → 若确无任何视觉模型则自动补写 zhipu-glm 声明 → 补写成功后立即重扫。
    // (修复:旧版补写后不再扫描,scanResult 一直停留在"无视觉模型",新机用户装完只见
    //  引导文案而看不到已自动加入的 zhipu-glm;重扫后路由/凭据状态立即就绪。)
    void (async () => {
      await scan()
      const wrote = await ensureZhipuProviderDeclaration()
      if (wrote) {
        log('info', 'startup', 'zhipu-glm 声明已补写,重新扫描模型列表…')
        await scan()
      }
    })()
    log('info', 'started', {
