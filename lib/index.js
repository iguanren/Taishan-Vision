...
function apply(ctx) {
    ...
    // 启动补写:仅当 llm-pi-ai.providers.zhipu-glm 完全缺失时自动写入(已存在则跳过,不覆盖用户自定义)
    ...
    function guidanceText(reason) {
      const lines = [
        ...
        '',
        '泰山识图推荐安装 GLM 免费视觉模型(智谱 zhipu-glm):',
        '- 完全免费:glm-4.6v-flash 与 glm-4.1v-thinking-flash',
        '- 支持文本 + 图像双模态,中文识别质量好',
        '',
        '配置文档网页:',
        '- 智谱开放平台:https://open.bigmodel.cn',
        '- 快速开始(首次 API 调用):https://docs.bigmodel.cn',
        '',
        '启用步骤:',
      ...
      if (reason === 'all-disabled') {
        ...
        lines.push('   推荐 provider: zhipu-glm,模型: glm-4.6v-flash(环境变量 ZHIPU_GLM_API_KEY);')
      }
      lines.push('3. 备用:glm-4.1v-thinking-flash 可作为第二视觉模型,在「设置 → 泰山识图」中调整路由;')
    ...
    function registerHttpApi() {
      ...
      ctx.effect(() => ws.register({
        ...
        handler: async (req, res) => {
          if (req.method === 'GET') {
            json(res, 200, { ok: true, credential: await describeCredential('ZHIPU_GLM_API_KEY') })
            return
          }
          if (!requirePost(req, res)) return
          let body = ''
          ...
          req.on('end', async () => {
            ...
            try {
              ...
              if (args && args.clear === true) {
                await credentialsSvc.unset('ZHIPU_GLM_API_KEY')
                reg.log('info', 'http-credential', '已清除 ZHIPU_GLM_API_KEY')
              } else if (args && typeof args.value === 'string' && args.value.trim() !== '') {
                await credentialsSvc.set('ZHIPU_GLM_API_KEY', args.value.trim())
                reg.log('info', 'http-credential', '已写入 ZHIPU_GLM_API_KEY(长度 ' + args.value.trim().length + ',值不明文记录)')
              } else {
              ...
              // 凭据变更后重扫,让模型凭据状态立即刷新
              await scan()
              json(res, 200, { ok: true, credential: await describeCredential('ZHIPU_GLM_API_KEY'), state: await panelState() })
            } catch (e) {
              json(res, 400, { ok: false, error: String(e), state: await panelState() })
            }
          })
    ...
    /* ==================== [12] 启动 ==================== */
    void ensureZhipuProviderDeclaration()
    void scan()
    log('info', 'started', {
      version: VERSION,
      tool: TOOL_NAME,
