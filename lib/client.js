...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function StatusCard(props) {
			...
			const checkUpdate = () => {
				...
				call("taishan/check-update", {})
					.then((res) => { if (res && res.checkedAt) onState({ ...state, update: res }); })
					.catch(() => {})
			...
			const updateBar = () => {
				...
				return React.createElement("div", { className: "jq-update-bar jq-update-bar-none" },
					React.createElement("span", null, "版本 " + ((update && update.current) || "v3.0") + (checking ? " · 检查中..." : " · 点击「检查更新」检测新版本")),
					React.createElement("button", {
		...
		function Panel() {
			...
			return React.createElement("div", { className: "jq-root" },
				React.createElement(StatusCard, { state, call, onState: setState }),
