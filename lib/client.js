...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			".jq-badge-dot{display:inline-flex;align-items:center;gap:6px}",
			...
			".jq-model-row{display:flex;align-items:center;gap:10px;height:36px;padding:0 10px;border-radius:8px;font-size:13px;cursor:default;transition:background .15s ease}",
			...
			".jq-key{font-size:12px;color:var(--success);flex:0 0 auto;margin-left:auto}",
			...
			".jq-update-bar{display:flex;align-items:center;gap:8px;padding:8px 12px;border-radius:8px;font-size:12px;margin-top:8px}",
			".jq-info-row{display:flex;align-items:center;gap:10px;margin-top:4px}",
			".jq-update-bar-none{background:var(--bg-subtle);color:var(--text-tertiary);border:1px solid var(--border-l2)}",
			".jq-update-bar-available{background:var(--brand-bg);color:var(--brand);border:1px solid var(--brand);cursor:pointer}",
			".jq-update-bar-available:hover{background:var(--brand-hover);color:#fff}",
			".jq-update-bar-checking{background:var(--bg-subtle);color:var(--text-secondary);border:1px solid var(--border-l2)}",
			".jq-update-bar-error{background:var(--error-bg);color:var(--error);border:1px solid var(--error)}",
			...
			".jq-route-handle{color:var(--text-tertiary);flex:0 0 auto;font-size:14px;cursor:grab;user-select:none}",
			...
			return React.createElement("div", { className: "jq-card" },
				React.createElement("div", { className: "jq-card-head jq-card-head-tight" },
					React.createElement("div", { className: "jq-log-side" },
						React.createElement("span", { className: "jq-card-title" }, "运行日志"),
		...
		function ModelsCard(props) {
			const { state, call, onState, mergeUpdate } = props;
			...
			// 合并「模型检测结果」+「识图路由」:每个模型一行,行尾 ⠿ 可拖动调序,
			// 行内只有小开关可启停(整行不可点击,避免误触)
			...
			const rows = shown.map((v) => {
				...
				return React.createElement("div", {
					...
					// 行尾拖动小点(与「识图路由」卡片同款 ⠿,放在 Key 状态后面)
					React.createElement("span", {
						className: "jq-route-handle",
						draggable: true,
			...
			return React.createElement("div", { className: "jq-card" },
				React.createElement("div", { className: "jq-card-head" },
					React.createElement("span", { className: "jq-card-title" }, "模型检测"),
					React.createElement("span", { className: "jq-tag" }, "启停 + 拖动调序"),
					React.createElement("span", { style: { marginLeft: "auto" }, display: "inline-flex", alignItems: "center", gap: "10px" },
						React.createElement("button", {
				...
				React.createElement("div", { className: "jq-section-title" }, "可用视觉模型（点 ⠿ 拖动可调整顺序,失败自动尝试下一个）"),
				vision.length === 0
		...
		function StatusCard(props) {
			...
			const checkUpdate = () => {
				...
				call("taishan/check-update", {})
					.then((res) => { if (res && res.checkedAt) mergeUpdate({ update: res }); })
					.catch(() => {})
					.finally(() => setChecking(false));
			};
			const doUpgrade = () => {
				setUpgradeToast(null);
				call("taishan/upgrade", {})
					.then((res) => {
						if (res && res.ok) {
							setUpgradeToast({ kind: "ok", text: "升级成功,请刷新浏览器" });
							setTimeout(() => { try { location.reload() } catch (_) {} }, 1500);
						} else {
							setUpgradeToast({ kind: "err", text: (res && res.error) || "升级失败" });
						}
					})
					.catch((e) => { setUpgradeToast({ kind: "err", text: String(e && e.message) }); })
					.finally(() => {});
			};
			// 泰山识图总开关:放在「上次扫描」信息行最前面
			const toggleEnabled = () => {
				call("taishan/panel/update", { patch: { enabled: !config.enabled } })
					.then((res) => { if (res && res.version) onState(res); })
					.catch(() => {});
			};
			// API Key 操作(合并自 CredentialCard)
			...
			const saveKey = () => {
				...
				call("taishan/panel/credential", { value: v })
					.then((res) => {
						...
						keyShowToast("ok", "API Key 已保存,凭据立即生效");
					})
			...
			const updateBar = () => {
				const hasUpdate = update && update.hasUpdate;
				const latest = update && update.latest;
				const error = update && update.error;
				// 「版本 v3.0 · 点击「检查更新」检测新版本」文案卡片已去掉,
				// 常规状态下不再渲染版本信息卡;仅保留 有新版/检查出错 两种提示条
				if (hasUpdate && latest) {
					return React.createElement("div", { className: "jq-update-bar jq-update-bar-available", onClick: doUpgrade, title: "点击升级" },
						React.createElement("span", null, "🔔"),
						React.createElement("span", null, "发现新版本 " + latest + " · 点击升级"),
					);
				}
				if (error) {
					return React.createElement("div", { className: "jq-update-bar jq-update-bar-error" },
						React.createElement("span", null, "更新检查失败,请手动访问 GitHub Releases"),
					);
				}
				return null;
			};
			return React.createElement("div", { className: "jq-card" },
				React.createElement("div", { className: "jq-card-head" },
					...
					// 整体靠右紧排:从右往左 = 检查更新 | 版本 | 图片通道 | API 状态
					React.createElement("span", { style: { marginLeft: "auto", display: "inline-flex", alignItems: "center", gap: "8px" } },
						cred.configured
							? React.createElement("span", { className: "jq-badge jq-badge-success" }, "API:已配置 ✅")
							: React.createElement("span", { className: "jq-badge jq-badge-subtle" }, "API:未配置 ❌"),
						runtime.wrapActive
						...
						React.createElement("span", { className: "jq-badge jq-badge-success jq-badge-dot" },
							...
							"版本：" + ((state && state.version) || "v4.0")),
						React.createElement("button", {
							className: "jq-btn jq-btn-ghost jq-btn-sm",
							disabled: checking,
							title: "检查新版本",
				...
				// 开关 + 上次扫描信息同一行
				React.createElement("div", { className: "jq-info-row" },
					React.createElement("span", { style: { fontSize: "12px", color: "var(--text-secondary)", flex: "0 0 auto", whiteSpace: "nowrap" } },
						config.enabled ? "停用泰山识图" : "启用泰山识图"),
					React.createElement("button", {
						className: "jq-switch" + (config.enabled ? " jq-on" : ""),
						onClick: toggleEnabled,
						role: "switch",
						"aria-checked": Boolean(config.enabled),
						title: config.enabled ? "点击关闭泰山识图自动流程" : "点击开启泰山识图自动流程",
						style: { flex: "0 0 auto" },
					}, React.createElement("span", { className: "jq-knob" })),
					React.createElement("span", { className: "jq-info-line", style: { margin: 0 } },
						"上次扫描:", " ",
				...
				// 分隔条 + API Key 配置行(合并自 CredentialCard)
				React.createElement("div", { style: { borderTop: "1px solid var(--border-l2)", margin: "10px 0 0", padding: "10px 0" } }),
				keyToast && React.createElement("div", { className: "jq-toast jq-toast-" + (keyToast.kind === "ok" ? "ok" : "err") }, keyToast.text),
				React.createElement("div", { style: { display: "flex", alignItems: "center", gap: "8px" } },
					React.createElement("span", { style: { fontSize: "12px", color: "var(--text-secondary)", flex: "0 0 auto" } }, "智谱 API KEY"),
					React.createElement("input", {
						className: "jq-cred-input", type: "password", value: keyValue,
					...
					React.createElement("button", {
						className: "jq-btn jq-btn-primary", onClick: saveKey, disabled: (keyValue || "").trim() === "",
					}, cred.configured ? "更新" : "保存"),
					React.createElement("button", { className: "jq-btn jq-btn-ghost", onClick: clearKey, disabled: !cred.configured }, "清除"),
				),
				upgradeToast && React.createElement("div", { className: "jq-toast jq-toast-" + upgradeToast.kind }, upgradeToast.text),
				updateBar(),
			);
		}
		...
		function Panel() {
			...
			// 函数式合并:只更新传入的字段,保留当前最新 state 的其他部分,
			// 避免闭包旧快照替换整个面板 state 导致的徽标闪动
			const mergeUpdate = React.useCallback((patch) => {
			...
			return React.createElement("div", { className: "jq-root" },
				React.createElement(StatusCard, { state, call, onState: setState, mergeUpdate }),
				React.createElement(ModelsCard, { state, call, onState: setState, mergeUpdate }),
